...
def extract_ctg_signals(image_path, fhr_top_ratio=0.55, bpm_per_cm=30, toco_per_cm=25,
    ...
    # Improved CTG check: FHR area must have variability (waveform)
    ...
    return fhr_signal, uc_signal, time_axis, True  # True = CTG image
...
def compute_model_features(fhr_signal, uc_signal, time_axis):
    ...
    # Fill other features as 0
    for col in ['Accelerations (SisPorto)', 'Uterine contractions (SisPorto)', 'Fetal movements (SisPorto)',
                'Light decelerations (raw)', 'Severe decelerations (raw)', 'Prolonged decelerations (raw)',
                'Repetitive decelerations (raw)',
                'Histogram width','Histogram minimum frequency','Histogram maximum frequency',
...
async def predict_ctg(file: UploadFile = File(...)):
    ...
    if not is_ctg:
        return {"prediction": 0, "label": "Error: Non CTG image has been scanned", "features": {}}
